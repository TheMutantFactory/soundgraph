...
named for itself — the tag list is the changelog. Numbered releases (`v0.1.1` was the
last before the show; `v0.2.0` is the Knobcon release) are a GitHub release on the tag
carrying the Windows editor, the web editor and the Axoloti card image, built by
`tools/package-release.py` after the tag exists (the build stamp reads `git describe`,
so a build before the tag names the wrong thing).
