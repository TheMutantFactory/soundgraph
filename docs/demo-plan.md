...
For the Axoloti with an Akai MPK mini on its host port there is a second set,
`examples/banks/axoloti-akai-mpk-mini.json`: 202 entries generated by
`tools/make-mpk-examples.py`, Poly Five first. Top row of knobs: pitch bend,
cutoff, resonance, an effect; bottom row: attack, decay, sustain, release. On
the instruments the pads are an arpeggiator — pads 1–7 a pattern each, pad 8
stop; hold a chord on Poly Five and it arpeggiates. The kit and the game sounds
have entries of their own with the pads as drums and sounds. Program Change is
the MPK's PROG CHANGE button plus a pad; the bank's "prev-next" mode makes pad 1
previous, pad 2 next and pad 3 home.
