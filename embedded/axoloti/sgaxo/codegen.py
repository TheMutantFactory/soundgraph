...
def _emit(nodes, bindings, order, frames, patch_id, events, zero_input,
          ...
          bank_nav=False, block_frames=64, capture_input=False):
    L = ["// Generated by sgaxo/codegen.py — do not edit.",
         f"#define SGAXO_FRAMES {int(block_frames)}",
         *(["#define SGAXO_CAPTURE_INPUT 1"] if capture_input else []),
         '#include "kernels.h"', ""]
    init = []
    note_nodes = []
    sdram_bytes = 0
...
def build_patch(patch_path, frames=4800, name=None, events=(),
                ...
                program_change="midi", block_frames=64, capture_input=False):
    ...
    block_frames: the graph's block. 64 is dsp-core's and what the golden
    captures compare against. The codec calls the patch every 16 frames and
    a 64-frame block renders inside one of those calls, so its peak load is
    four times its mean: Poly Five at a 74% mean overran every call and
    sounded chunky. Banks bake at 16, where peak is mean; per-block
    modulation (an SVF's cutoff sweep) then updates four times as often,
    which is finer than the desktop, not coarser.
    ...
    source = _emit(nodes, bindings, order, frames, pid, list(events),
                   ...
                   block_frames=block_frames, capture_input=capture_input)
